...
impl ExternalCommand {
    pub fn run_with_input(
        ...
        let mut process = self.create_process(&input, false, !self.redirect_stdout, head)?;
        let child;
                ...
                if !input.is_nothing() {
                    ...
                    if let Some(mut stdin_write) = child.stdin.take() {
                        std::thread::spawn(move || {
                            let input = nu_command::Table::run(
                                &nu_command::Table,
                                &engine_state,
                                &mut stack,
                                &Call::new(head),
    ...
    fn create_process(
        ...
        let mut process = if let Some(d) = self.env_vars.get("PWD") {
            let mut process = if use_cmd {
                self.spawn_cmd_command(d, use_external_terminal)
            } else {
                self.create_command(d, use_external_terminal)?
            };
    ...
    fn create_command(
        ...
        if cfg!(windows) {
            ...
            if self.name.item.ends_with(".cmd") || self.name.item.ends_with(".bat") {
                Ok(self.spawn_cmd_command(cwd, use_external_terminal))
            } else {
                self.spawn_simple_command(cwd, use_external_terminal)
            }
        } else if self.name.item.ends_with(".sh") {
            Ok(self.spawn_sh_command(cwd, use_external_terminal))
        } else {
            self.spawn_simple_command(cwd, use_external_terminal)
        }
    }
    ...
    pub fn spawn_simple_command(
        &self,
        cwd: &str,
        use_external_terminal: bool,
    ) -> Result<std::process::Command, ShellError> {
        let is_path = |arg: &str| {
        ...
        let mut process = if use_external_terminal && head != "alacritty" {
